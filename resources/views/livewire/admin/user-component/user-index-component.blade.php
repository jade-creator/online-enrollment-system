<div class="w-full">
    ...
    <div class="h-content w-full p-4 md:p-8">
        ...
        <x-table.main>
            ...
            <x-slot name="paginationLink">{{ $users->links('partials.pagination-link') }}</x-slot>
            ...
            <x-slot name="head">
                ...
                <div class="col-span-3"><x-table.sort-button event="sortFieldSelected('email')">email</x-table.sort-button></div>
                            ...
                            <div name="slot" class="grid grid-cols-12 md:gap-2">
                                ...
                                <x-table.cell headerLabel="Name" class="justify-start md:col-span-3 md:flex items-center">
                                    @if ( Laravel\Jetstream\Jetstream::managesProfilePhotos() )
                                        <div class="hidden md:block mr-4 flex-shrink-0"><img class="h-8 w-8 rounded-full object-cover" src="{{ $user->profile_photo_url }}"/></div>
                                    @endif
                                    <div class="py-4">{{ $user->name ?? 'N/A'}}</div>
                                </x-table.cell>
                                <x-table.cell headerLabel="Email" class="justify-start md:col-span-3">{{ $user->email ?? 'N/A' }}</x-table.cell>
                                <x-table.cell headerLabel="Role" class="justify-start md:col-span-3">{!! $user->role->name_element ?? 'N/A' !!}</x-table.cell>
                                <x-table.cell-action>
                                    @if (!count($selected) > 0)
                                        <x-jet-dropdown align="right" width="60" dropdownClasses="z-10 shadow-2xl">
                                            ...
                                            <x-slot name="content">
                                                <div class="w-60">
                                                    ...
                                                    <a href="{{ route('user.personal.profile.view', $user->id) }}">
