<div class="w-full flex flex-1 scrolling-touch">
    ...
    <div class="min-h-screen w-full py-8 px-8">
        ...
        <x-table.title tableTitle="Types" :isSelectedAll="$this->selectAll" :count="count($this->checkedTypes)" :route="route('admin.users.create')" routeTitle="Add Type"/>
        ...
        <x-table.main>
            ...
            <x-slot name="head">
                <div class="col-span-6 flex" id="columnTitle">
                    ...
                    <x-table.sort-button nameButton="type" event="$emitSelf('sortFieldSelected', 'type')"/>
                </div>
                <x-table.column-title columnTitle="created" class="col-span-6"/>
            </x-slot>

            <x-slot name="body">
                @if ($types->count())
                    @foreach ($types as $type)
                        <div class="w-full p-2 my-1 rounded-md shadow hover:shadow-md @if ($this->isSelected($type->id)) bg-gray-200 @else bg-white @endif border-t border-l border-r border-gray-200 border-opacity-80">
                            <div class="grid grid-cols-12 gap-2">
                                <div class="col-span-12 md:col-span-6 truncate font-bold text-xs">
                                    <div class="flex items-center">
                                        <input type="checkbox" id="{{ $type->id }}" value="{{ $type->id }}" wire:model="checkedTypes" class="cursor-pointer border-gray-500 border-opacity-50 focus:outline-none focus:ring focus:ring-transparent ml-3 mr-5 rounded-sm">
                                        <div class="h-10 ml-4 flex items-center">
                                            {{ $type->type }}
                                        </div>
                                    </div>
                                </div>
                                <div class="flex items-center justify-start col-span-12 md:col-span-5 truncate md:border-0 border-t border-gray-300 font-bold text-xs">{{ $type->created_at }}</div>
                    ...
                    @endforeach
                @else
                    <x-table.no-result title="No type found.🤔"/> 
                @endif
            </x-slot>
        </x-table.main>
    ...
    <div wire:loading wire:target="paginateValue, search, selectPage, previousPage, nextPage, sortFieldSelected">
