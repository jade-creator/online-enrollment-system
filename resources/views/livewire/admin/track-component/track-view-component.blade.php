<div class="w-full flex flex-1 scrolling-touch">
    ...
    <div class="min-h-screen w-full py-8 px-8">
        ...
        <div class="mb-4 pb-3 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div class="flex items-center justify-between">
                    <div class="text-2xl font-bold text-gray-500">Tracks</div>
                ...
                <button wire:click="$toggle('addingTrack')" wire:loading.attr="disabled" class="focus:ring-2 focus:bg-blue-500 focus:ring-opacity-50 bg-blue-500 hover:bg-blue-600 text-white py-2.5 px-4 font-bold text-xs rounded-lg border border-white">Add Track</button>
            </div>
        </div>
        ...
        <x-table.main>
            ...
            <x-slot name="body">
                            ...
                            <div class="col-span-12 md:col-span-5 truncate font-bold text-xs">
                                <div class="flex items-center">
                                    ...
                                    <div class="h-10 flex items-center">
                                        <p class="truncate">{{ $track->track ?? 'N/A' }}</p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <x-jet-dropdown align="right" width="60" dropdownClasses="z-10 shadow-2xl">
                                        ...
                                        <x-slot name="content">
                                            <div class="w-60">
                                                ...
                                                <div>
                                                    <button wire:click.prevent="viewTrack({{ $track }})" class="flex w-full px-4 py-2 hover:bg-gray-200 outline-none focus:outline-none transition-all duration-300 ease-in-out">
                                                ...
                                                <div>
                                                    <button wire:click.prevent="removeConfirm({{ $track }})" class="flex w-full px-4 py-2 hover:bg-red-500 hover:text-white rounded-b-md outline-none focus:outline-none transition-all duration-300 ease-in-out">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                ...
                @empty  
                    <x-table.no-result title="No track found.🤔"/> 
                @endforelse
            </x-slot>
        </x-table.main>
    ...
    <div wire:loading wire:target="paginateValue, search, selectPage, previousPage, nextPage, confirmingExport, viewTrack, removeConfirm">
    ...
    <!-- Add Track's Modal -->
    <x-jet-dialog-modal wire:model="addingTrack">
        <x-slot name="title">
            {{ __('Track Maintenance') }}
        </x-slot>
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('addingTrack')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>
    ...
    <!-- Add Track's Modal -->
    <x-jet-dialog-modal wire:model="viewingTrack">
        <x-slot name="title">
            {{ __('Track Maintenance') }}
        </x-slot>
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('viewingTrack')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2 bg-blue-500 hover:blue-700" wire:click="updateTrack" wire:loading.attr="disabled">
                {{ __('Update') }}
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
