                ...
                <div wire:loading.class="hidden" wire:target="search" class="w-full">
                    @if (empty($employee))
                        @forelse ($professors as $professor)
                            <button wire:click.prevent="$set('employee', {{$professor}})" class="w-full flex items-center justify-between px-2 py-3 hover:bg-gray-50 rounded-md focus:outline-none">
                            <span class="flex items-center">
                                @if ( Laravel\Jetstream\Jetstream::managesProfilePhotos() )
                        ...
                        @if (filled($professors))
                            {{ $professors->links('partials.pagination-link') }}
                        @endif
                    @else
