...
class RegistrationAddComponent extends Component
{
    ...
    {
        return [
            'registration.isNew' => ['required', 'integer'],
            'programId' => ['required'],
            'levelId' => ['required'],
            'termId' => ['required'],
        ];
    ...
    public function next()
    {
        ...
        try {
            $this->prospectus = (new ProspectusSubjectService())->register($this->programId, $this->levelId, $this->termId);
            if ($this->selectAll) $this->pluckRows($this->prospectus->subjects);\Debugbar::info($this->selected);
            $this->currentStep++;
        } catch (\Exception $e) {
            $this->error($e->getMessage());
        }
